...
function drag($start_e, $end_e)
{
	...
	$result1 = mysql_query("SELECT * FROM goods WHERE number='$start_e'");
	$myrow1 = mysql_fetch_assoc($result1); 
	$start_e_id = $myrow1[id];
	
	//запрос к базе, получаем следующий объект
	$result2 = mysql_query("SELECT * FROM goods WHERE number='$end_e'");
	...
	{	
		...
		$result4 = mysql_query("SELECT * FROM goods WHERE number>'$start' && number<='$end' ORDER BY number ASC");
		...
		//редактирование
		$result_edit = mysql_query("UPDATE goods SET number='$end_e' WHERE id='$start_e_id'", $db);		
	}
	else
	{
		$start = $start_e;
		...
		$result4 = mysql_query("SELECT * FROM goods WHERE number>='$end' && number<'$start' ORDER BY number ASC");
		$myrow4 = mysql_fetch_assoc($result4); 
