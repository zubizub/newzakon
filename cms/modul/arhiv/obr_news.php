...
if (isset($_GET[del]))
{
	...
	//удаление
	$del = mysql_query ("DELETE FROM news WHERE id = '$del'",$db);
...
if (!isset($_POST[edit]))
{
	...
	//добавление
	$result_add = mysql_query ("INSERT INTO news (name,date,text,form,form_position,enabled,m_title,m_description,m_keywords,m_link,number) VALUES ('$name','$date','$text','$form','$form_position','$enabled','$m_title','$m_description','$m_keywords','$m_link','$number')");	
...
{		
	...
	//запрос к базе, - задание номера товара
	$result = mysql_query("SELECT * FROM news WHERE id='$edit'");
	$myrow = mysql_fetch_assoc($result);
	...
	{
		$result1 = mysql_query("SELECT * FROM news WHERE id='".($edit-1)."'");
		$myrow1 = mysql_fetch_assoc($result1);				
		$p_last_e = $myrow1[id];
		
		$result2 = mysql_query("SELECT * FROM news WHERE id='".($edit+1)."'");
		$myrow2 = mysql_fetch_assoc($result2);		
		$p_next_e = $myrow2[id];
		
		$result3 = mysql_query("SELECT * FROM news WHERE id='$edit'");
