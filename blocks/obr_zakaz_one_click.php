...
{
	Header("location:/buy/$one_click/?name=$name&phone=$phone&mail=$mail&text=$text&msg=Не заполнены обязательные поля!");
	exit;
}
...
//добавление
$result_add = mysql_query ("INSERT INTO zakaz (fio,price,mail,phone,address,oplata,status,text,dostavka,date,kupon,ip,time) VALUES ('$name','$summa_zakaz','$mail','$phone','$address','0','на обработке','$text','$dostavka','$date','$kupon','$ip','$time')");
...
VALUES ('$zakaz_id','$myrow_g[name]','$myrow_g[id]','$myrow_g[art]','$price','1','$myrow_g[sale]')");	
